...
contract RentalityPaymentService is UUPSOwnable {
  ...
  function payKycCommission(address user, uint valueInCurrency, address currencyType) public payable {
    require(userService.isManager(msg.sender), 'Only manager');
  ...
  function payCreateTrip(address user, address currencyType, uint valueSumInCurrency) public payable {
    require(userService.isManager(msg.sender), 'only manager');
  ...
  function payClaim(address user, Schemas.Trip memory trip, uint valueToPay, uint feeInCurrency, uint commission) public payable {
    require(userService.isManager(msg.sender), 'Only manager');
